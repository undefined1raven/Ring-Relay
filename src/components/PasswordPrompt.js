...
function PasswordPrompt(props) {
    ...
    const enter = () => {
        if (input.length > 5) {
            axios.post(`${DomainGetter('prodx')}api/dbop?verifyPassword`, { rtdbPayload: props.rtdbPayload, password: input, exportType: props.exportType, AT: localStorage.getItem('AT'), CIP: localStorage.getItem('CIP') }).then(res => {
                if (res.data.flag) {
                    props.onValid();
