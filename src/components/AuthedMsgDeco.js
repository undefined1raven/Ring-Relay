...
function AuthedMsgDeco(props) {
    ...
    const primaryColorController = () => {
        let colors = {};
        if (props.signed == 'self' || props.signed === true || props.signed == undefined) {
            if (props.ghost) {
                colors = { foreground: '#FFF', bkg: '#0500FF' };
            } else {
                colors = { foreground: '#00FFD1', bkg: '#00FFD1' };
            }
        } else {
            colors = { foreground: '#FF9900', bkg: '#FF9900' };
        }
        if (props.isImage) {
            colors = {...colors, bkg: '#000000'}
        }
        return colors;
    }
